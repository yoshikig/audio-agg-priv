...
impl TimeSyncEstimator {
    pub fn new(alpha: f64, beta: f64) -> Self {
        Self { alpha, beta, last_offset_ms: None, last_t3_ms: None, state: Default::default() }
    }

    // NTP-like: t0=client send, t1=server recv, t2=server send, t3=client recv
    pub fn update(&mut self, t0_ms: u64, t1_ms: u64, t2_ms: u64, t3_ms: u64) -> TimeSyncState {
        let t0 = t0_ms as f64;
        let t1 = t1_ms as f64;
        let t2 = t2_ms as f64;
        let t3 = t3_ms as f64;
        ...
        if self.last_offset_ms.is_none() {
            ...
            self.state.delay_ms = (1.0 - a) * self.state.delay_ms + a * delay.max(0.0);
        }

        // Drift as change in offset over change in t3
        if let (Some(prev_off), Some(prev_t3)) = (self.last_offset_ms, self.last_t3_ms) {
