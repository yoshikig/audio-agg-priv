...
impl core::fmt::Display for SyncDecodeError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            SyncDecodeError::BadVersion => write!(f, "unsupported sync packet version"),
...
pub fn decode_sync(data: &[u8]) -> Result<SyncMessage, SyncDecodeError> {
    ...
    if data.len() < 3 { return Err(SyncDecodeError::TooShort); }
    match data[2] {
        TYPE_PING => {
            if data.len() < 3 + 8 { return Err(SyncDecodeError::TooShort); }
            let mut b = [0u8; 8];
            b.copy_from_slice(&data[3..11]);
            Ok(SyncMessage::Ping { t0_ms: u64::from_be_bytes(b) })
        }
        TYPE_PONG => {
            if data.len() < 3 + 8 + 8 + 8 { return Err(SyncDecodeError::TooShort); }
            let mut b0 = [0u8; 8];
            let mut b1 = [0u8; 8];
            let mut b2 = [0u8; 8];
