...
impl core::fmt::Display for DataPacketError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            DataPacketError::BadVersion => write!(f, "unsupported data packet version"),
            DataPacketError::LengthMismatch => write!(f, "declared length exceeds buffer"),
        }
    }
}
...
pub fn decode_packet<'a>(data: &'a [u8]) -> Result<Decoded<'a>, DataPacketError> {
    if data.len() < HEADER_LEN { return Err(DataPacketError::TooShort); }
    if data[0] != DATA_PACKET_MAGIC { return Err(DataPacketError::BadMagic); }
    if data[1] != PACKET_VERSION { return Err(DataPacketError::BadVersion); }
    ...
    if data.len() < HEADER_LEN + payload_len { return Err(DataPacketError::LengthMismatch); }
...
mod tests {
    ...
    fn enforces_length_and_magic_version() {
        ...
        assert_eq!(decode_packet(&bad_version), Err(DataPacketError::BadVersion));
